...
export class AuthService {
  ...
  uid: any = new BehaviorSubject(null)
  constructor(private fireAuth: AngularFireAuth, public Router: Router, private _TranslateService: TranslateService) { }
  ...
  async register(email: string, password: string) {
    await this.fireAuth.createUserWithEmailAndPassword(email, password).then((res) => {
      this.registerErrors = ''
      this.Router.navigate([this._TranslateService.currentLang, 'login'])
  ...
  login(email: string, password: string) {
    this.fireAuth.signInWithEmailAndPassword(email, password).then((res) => {
      this.loginErrors = ''
      // this.toastr.success('Welcome to SwiftCart', '', {
      //   progressBar: true,
      // });
      console.log(res)
      ...
      this.Router.navigate([this._TranslateService.currentLang])
    }).catch(error => {
